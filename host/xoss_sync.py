...
class BluetoothFileTransfer:
    ...
    async def run(self):
        ...
        async with BleakClient(device.address, timeout=60.0) as client:
            if client.is_connected:
                ...
                # G-933835 uses the JSON index used by newer XOSS models.
                track_index = 'workouts.json'
                await self.fetch_file(client, track_index)
                fit_files = self.extract_fit_filenames(track_index)
    ...
    def save_file_raw(self, filename):
        with open(filename, "wb") as file:
            # YMODEM padding is outside the declared file size; a real FIT file
            # may itself end in a zero byte, so trim by the protocol length.
            size = file.write(self.data[:self.data_size])
